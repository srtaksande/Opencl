...
int main()
{
	...
	Aa = (int *)malloc(ARRSIZE*sizeof(int));
	...
	Aad = clCreateBuffer(context, CL_MEM_WRITE_ONLY, ARRSIZE * sizeof(int), NULL, &ret);
	...
	ret = clSetKernelArg(kernel, 1, sizeof(cl_mem), &Aad);
	...
	// Copy results back
	ret = clEnqueueReadBuffer(command_queue, Aad, CL_TRUE, 0, ARRSIZE*sizeof(int), Aa, 0, NULL, NULL);
